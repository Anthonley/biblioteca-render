-- Biblioteca Municipal - esquema para MySQL (Clever Cloud / phpMyAdmin)
-- Refleja exactamente la misma estructura y datos que biblioteca_post.sql (PostgreSQL/Render),
-- adaptada a sintaxis MySQL.
--
-- IMPORTANTE sobre acentos/ñ: al importar este archivo en phpMyAdmin, asegúrate de que
-- la opción de "Conjunto de caracteres del archivo" (charset del import) esté en utf8mb4,
-- y que la base de datos/tablas queden creadas con utf8mb4_general_ci o utf8mb4_unicode_ci
-- (ver el CREATE TABLE de abajo, ya lleva CHARACTER SET utf8mb4 explícito). Ese es justamente
-- el problema que tuviste en Render: si el archivo se sube con un charset distinto al de la
-- tabla, los tildes/ñ quedan mal codificados (Ciencia FicciÃ³n en vez de Ciencia Ficción).
...
DROP TABLE IF EXISTS sede_biblioteca;
DROP TABLE IF EXISTS usuario;

SET FOREIGN_KEY_CHECKS = 1;

-- id_sede es manual con formato SE-0000 (ver sedes.php / sedes_guardar.php)
...
-- id_genero es manual con formato GE-0000 (ver generos_guardar.php)
CREATE TABLE genero (
    id_genero VARCHAR(10) PRIMARY KEY,
    ge_nombre VARCHAR(100) NOT NULL UNIQUE
) ENGINE=InnoDB CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;

-- id_tag es manual con formato TA-0000 (ver libros_guardar.php)
CREATE TABLE tag (
...
-- id_socio es manual con formato SO-0000 (ver socios.php / socios_guardar.php)
CREATE TABLE socio (
...
-- id_ejemplar es manual con formato EJ-0000 (ver ejemplares.php / ejemplares_guardar.php)
...
-- id_prestamo es manual con formato PR-0000 (ver prestamos.php / prestamos_guardar.php)
...
CREATE TABLE prestamo (
    ...
    pr_multa DECIMAL(8,2) NOT NULL DEFAULT 0,
...
-- usuario se mantiene igual: id numérico autoincremental
